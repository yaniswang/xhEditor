...
{
	$result = false;
	if(! is_dir($dirName))trigger_error("目录名称错误", E_USER_ERROR);
...
if($err==''){
	...
	if(preg_match('/^('.str_replace(',','|',$upExt).')$/i',$extension))
	{
		...
		else
		{
			...
			$lastday_dir= $attachDir.'/'.'day_'.date('ymd',strtotime("7 days ago"));
			if(is_dir($lastday_dir))removeDir($lastday_dir);
